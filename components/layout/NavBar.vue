  ...
  <nav>
    <div class="navbar">
      ...
      <div class="navbar-item is-desktop">
        <nuxt-link :to="localePath('/')" class="line-hover">{{
          $t("home")
        }}</nuxt-link>

        <nuxt-link class="line-hover" :to="localePath('/casino')">{{
          $t("casino")
        }}</nuxt-link>
        <div
          class="has-dropdrown"
          @mouseover="
            drowdownToggle = true;
            drowdownToggle2 = false;
            showMoreDropDown = false;
          "
        >
          <nuxt-link :to="localePath('/hotels')" class="line-hover cursor">
            {{ $t("hotels") }}
            <DownOutlined
          ...
          >
            <li v-for="(i, idx) in main.hotels" :key="idx">
              <nuxt-link
                ...
                class="line-hover"
                >{{ i.titles[selectedIdx] }}</nuxt-link
              >
            </li>
          </ul>
        </div>
        ...
        <div
          class="has-dropdrown"
          @mouseover="
            drowdownToggle2 = true;
            drowdownToggle = false;
            showMoreDropDown = false;
          "
        >
          <nuxt-link :to="localePath('/experience')" class="line-hover cursor">
            {{ $t("experience") }}
            <DownOutlined
              style="position: absolute; top: 3px; right: -18px"
              v-if="drowdownToggle2"
            />
          </nuxt-link>
          <ul
            @click="drowdownToggle2 = false"
            class="dropdrown"
            style="width: 310px !important"
            v-if="drowdownToggle2"
          ...
          >
            <li v-for="(i, idx) in blogs" :key="idx">
              <nuxt-link
                :to="
                  localePath(
                    '/experience/' + i._id + '?title=' + i.titles[selectedIdx]
                  )
                "
                @click="drowdownToggle2 = !drowdownToggle2"
                class="line-hover"
                >{{ i.titles[selectedIdx] }}</nuxt-link
              >
            </li>
        ...
        <nuxt-link :to="localePath('/events')" class="line-hover">
          {{ $t("events") }}
        </nuxt-link>

        <nuxt-link :to="localePath('/offer')" class="line-hover">{{
          $t("offer")
        }}</nuxt-link>
        <nuxt-link class="line-hover" :to="localePath('/new')">
          New page
        </nuxt-link>
        <dividerProps class="has-dropdrown">
          ...
          >
            <li>
              <nuxt-link class="line-hover" :to="localePath('/near-by')">
                {{ $t("near By") }}
              </nuxt-link>
            </li>
            <li>
              <nuxt-link class="line-hover" :to="localePath('/tour-package')">{{
                $t("tour Package")
              }}</nuxt-link>
            </li>
            <li>
              <nuxt-link class="line-hover" :to="localePath('/gallery')">{{
                $t("gallery")
              }}</nuxt-link>
            </li>
            <li>
              <nuxt-link :to="localePath('/more/career')" class="line-hover">{{
                $t("careerTitle")
              }}</nuxt-link>
            </li>
            ...
            <li>
              <nuxt-link :to="localePath('/news')" class="line-hover">{{
                $t("newsTitle")
              }}</nuxt-link>
            </li>
          </ul>
        ...
        <nuxt-link :to="localePath('/contact')" class="line-hover">
          {{ $t("contact") }}
        </nuxt-link>
      </div>
